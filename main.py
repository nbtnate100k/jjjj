...
def reload_web_touched(data: dict) -> None:
    """Reload records the pluxo.net API may have updated while the bot runs."""
    data["admin_users"] = load_json(USERS_FILE, {})
    data["purchases"] = load_json(PURCHASES_FILE, [])
...
def _run_bot_forever() -> None:
    """Keep the Telegram bot alive. If it crashes (e.g. getUpdates conflict), wait and retry so the HTTP API stays up."""
    import asyncio
    import time

    # Background threads don't have an asyncio loop by default; PTB's run_polling() needs one.
    ...
    while True:
        try:
            ...
            application.run_polling(
                ...
                close_loop=False,
                stop_signals=None,  # signals only work on the main thread
            )
            ...
            logger.error("Telegram bot crashed: %s. Restarting in %ss.", exc, backoff)
            # After a crash, give ourselves a fresh loop on the next retry.
            try:
                asyncio.set_event_loop(asyncio.new_event_loop())
            except Exception:
                pass
...
def main():
    """Entrypoint: HTTP API in main thread (so Railway health-checks keep working), Telegram bot in background."""
    run_http_in_background = os.getenv("API_IN_THREAD", "").lower() in ("1", "true", "yes")

    if run_http_in_background:
        threading.Thread(target=run_server, daemon=True, name="PluxoAPI").start()
        _run_bot_forever()
    else:
        threading.Thread(target=_run_bot_forever, daemon=True, name="PluxoBot").start()
        run_server()
